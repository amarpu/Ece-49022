...
// This function prints status updates for WiFi events to the Serial Monitor.
...
// Initializes and connects to the WPA2-Enterprise WiFi network.
// It handles authentication and waits for a successful connection.
void setupWiFi() {
  // WiFi Credentials
  const char* ssid = "PAL3.0";
  const char* eap_username = "yu1206";      // Your EAP username
  const char* eap_password = "xxxxxxxxxx";  // Your EAP password
  ...
  WiFi.mode(WIFI_STA);
  WiFi.onEvent(WiFiEvent); // Register our event handler
...
// Initializes the web server and maps URL endpoints to handler functions.
void setupWebServer() {
